...
function DecisionCard({ decision }) {
  ...
  return <Alert className="decision-card" type={type} showIcon
    ...
    description={<><Collapse size="small" ghost items={[{ key: 'reasoning', label: 'AI Reasoning', children: <span>{decision.reasoning}</span> }]} /><strong>Fraud risk:</strong> {decision.fraudRisk}</>} />;
}
