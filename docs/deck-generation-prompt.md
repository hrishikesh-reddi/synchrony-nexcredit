...
3. **Secure and human-in-the-loop**   JWT RBAC; uncertain/policy-sensitive cases route to a reviewer.

### Slide 4   Architecture (KEY SLIDE)
Render this diagram (boxes and arrows), title "How it works":
```
React workbench (role-aware UI)
        │  HTTPS and JWT Bearer
        ▼
Spring Security (JWT, roles: APPLICANT/UNDERWRITER/ADMIN)
Spring Boot REST API
   ├─ Underwriting Engine (rules and bias guardrail)
   ├─ AI Layer: EmbeddingService · VectorStore(pgvector) · ExplanationService(guarded)
   └─ Audit and Review Queue
        │
        ▼
PostgreSQL and pgvector (evidence_embedding)
```
Caption: "The AI layer sits *beside* the rules engine, not above it."

### Slide 5   Multi-modal underwriting flow
Two-path flow:
- Path A (structured): signals  to  creditworthiness  to  fraud  to  decision  to  explanation  to  audit.
- Path B (unstructured): document upload  to  Tika extract  to  embed  to  pgvector index  to  semantic search.
...
- "Guardrails in code, not prompts"   `guardrailOk()` and `sanitize()` server-side.
...
- Guardrails in code (prompt constraints and server-side injection defence)
