...
- Provides a **human-review queue** + **per-decision audit log** for accountability.
- Lets underwriters **upload documents** (Apache Tika extraction) and run **pgvector semantic
  search** across evidence.
- Offers an **LLM explanation assistant** (optional OpenAI-compatible API + deterministic local
  fallback, with guardrails) that explains any decision in plain language.
- Secures the API with **JWT auth + roles** (applicant / underwriter / admin).

All of this runs on a standard Spring Boot 3 + React 18 + PostgreSQL/pgvector stack, with 13/13
tests green.
...
while these external capabilities are wired in (research-grounded, India + US contexts):
...
| **Decisioning orchestration** | Combine sources into one policy + reason codes | Scienaptic-style decisioning layer |
...
- **Richer evidence viewer:** inline document preview + highlighted semantic matches from
...
1. **Landing** — problem statement + "contextual underwriting" pitch.
2. **Login** as underwriter (demo creds auto-filled).
3. **New application** — fill a sample NTC applicant, hit *Analyze*; show the decision + confidence
   + fraud-risk + stage trace.
...
5. **Document upload + semantic search** — upload an income-evidence file, then run a semantic
