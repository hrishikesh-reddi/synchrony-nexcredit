...
## Status: READY FOR SUBMISSION (pending PDF export and demo video and email)

## Completed work

### Rebrand / de-scaffolding (done)
- Renamed Java package `com.syscomz.springbootfullstackprofessional`  to  `com.synchrony.nexcredit`.
- Renamed main class `SpringBootFullStackProfessionalApplication`  to  `NexCreditApplication`;
  test `SpringBootFullStackProfessionalApplicationTests`  to  `NexCreditApplicationTests`.
...
- Rewrote `HELP.md` and CI (`.github/workflows/build.yml`) under the NexCredit identity.

### Backend features (done, 13/13 tests green)
- Spring Boot 3.3.5 and Java 17, `javax` to `jakarta`.
- Spring Security and JWT: `SecurityConfig`, `JwtUtil`, `JwtAuthenticationFilter`, `AuthController`
...
- LLM `ExplanationService` (OpenAI-compatible optional and deterministic local fallback, guardrails).
- **Strengthening (2026-08-21):** `POST /api/credit/applications` now runs the underwriting policy
  on save, so new applications get a decision and correct review routing (bias guardrail included).
...
- `docs/product-vision.md`   problem statement (sell), Synchrony PRISM mapping, SaaS expansion table
  ...
  Use for README overview and pitch.
- `submission/SE23UCSE065.zip` regenerated (363K) from current code and roll-numbered docs.

### In progress (separate autonomous terminal/agent)
- **Screenshots into README**: an agent will boot pgvector (5434) and backend (8081, `DB_PASSWORD=admin`)
 and frontend (`REACT_APP_API_BASE_URL=http://localhost:8081`), capture landing/workbench/dashboard/
...
- **Ollama not used**; LLM layer uses OpenAI-compatible API (optional) and local rule-based fallback.
- **Spring AI not used**; pgvector and RestClient used directly.
- Roles are `APPLICANT / UNDERWRITER / ADMIN` (checklist said "auditor"; ADMIN ~ auditor).

## Remaining before submit (owner: user)
- Export `submission/SE23UCSE065/SE23UCSE065-pdf-source.md`  to  `SE23UCSE065.pdf`.
- Optional: drop recorded `SE23UCSE065-demo.mp4` into `submission/SE23UCSE065/` and re-run
  `bash build-submission.sh`.
- Email ZIP and PDF to Technologyinterns@syf.com before 12:00 PM IST.
